...
library SmartCodec {
    ...
    function encodeMarkup64(
        ...
        return
            string(
                abi.encodePacked("data:text/html;base64,", encode(bytes(data)))
            );
    }
    ...
    function processMarkup() internal pure returns (string[] memory markups) {
        ...
        markups[0] = string(abi.encodePacked(decode(MARKUP1)));
        markups[1] = string(abi.encodePacked(decode(MARKUP2)));
        markups[2] = string(abi.encodePacked(decode(MARKUP3)));
        markups[3] = string(abi.encodePacked(decode(MARKUP4)));
        markups[4] = string(abi.encodePacked(decode(MARKUP5)));
    }
}
