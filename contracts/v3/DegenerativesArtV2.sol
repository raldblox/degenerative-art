...
// File: contracts/interfaces/IDegenerativesArt.sol
...
// File: contracts/interfaces/IVisualEngine.sol
...
// File: contracts/DegenerativesArt.sol
...
// File: contracts/DegenerativesArtV2.sol
...
contract DegenerativesArtV2 is IDegenerativesArt, ERC721, Ownable(msg.sender) {
    DegenerativesArt public degenerativesV1;
    ...
    constructor() ERC721("degeneratives.art", "DEGENARTV2") {
        degenerativesV1 = DegenerativesArt(
    ...
    modifier validEmojis(string[] memory _emojis) {
        require(
            _emojis.length >= 3 && _emojis.length <= maxEmojis,
            "Invalid qty. of emojis."
        );
        _;
    }
    ...
    function migrate(uint256 tokenId) external payable onlyOwner {
        require(migrating, "Migrate finished.");
        // migrate main emojis
        string[] memory emojis_ = degenerativesV1.getEmojis(tokenId);
        address v1owner = degenerativesV1.ownerOf(tokenId);
        ...
        _updateEmojis(v1owner, totalSupply, emojis_);
    ...
    function updateMigrationStatus(bool isMigrating) external onlyOwner {
        migrating = isMigrating;
    }

    function updateBurningStatus(bool isMigrating) external onlyOwner {
        migrating = isMigrating;
    }
